...
export default class PromotionMongooseRepository implements PromotionInterface {
    async createPromotion(promotion: Promotion): Promise<any> {
        try {
            ...
            if (result && result.insertedId) {
              return {id:result.insertedId,name:promotion.name,description:promotion.description,type:promotion.type,value:promotion.value,code:promotion.code}
    ...
    async searchPromotion(code: string): Promise<any> {
        try {
            ...
            if (product) {
                return {name: product.name, desccription: product.description,type:product.type,value:product.value,code:product.code, id:product._id}
    ...
    async listAllPromotions(): Promise<any> {
        try {
            ...
            if (promotions && promotions.length>0) {
              ...
              return promotions.map((promotion) => new Promotion( promotion.name, promotion.description,promotion.type, promotion.value, promotion.code, promotion._id));
            } else {
